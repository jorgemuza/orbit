...
Secrets are resolved at runtime via `op read`, so the [1Password CLI](https://developer.1password.com/docs/cli/) must be installed and authenticated. Resolved values are cached in-process to avoid repeated biometric prompts.

Pre-resolve secrets before running multiple commands:

```bash
orbit auth          # triggers biometric once, caches for the session
```
